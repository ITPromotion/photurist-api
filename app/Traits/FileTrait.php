            ...
            if (isset($type) && MediaContentType::PHOTO == $type) {
                ...
                foreach (SizeImage::keys() as $value) {
                    try {
                        ...
                        $video->filters()->custom("scale=w=$scaleW:h=$scaleH,crop=$size:$size")->addFilter(new \FFMpeg\Filters\Audio\SimpleFilter(['-an']))->clip(TimeCode::fromSeconds(Video::START), TimeCode::fromSeconds(1));
